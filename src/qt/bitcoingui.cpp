...
#define VERTICAL_TOOBAR_STYLESHEET "QToolBar {border:none; height:100%; padding-top:20px; text-align: left; background: 1px solid;}\
QToolButton {min-width:180px;background-color: transparent;border: 0px solid #FAFAFA;border-radius: 3px;margin: 3px;padding-left: 5px;padding-top:5px;width:100%;text-align: left;padding-bottom:5px;border-left-width: 0px;color: rgb(92,81,92);}"

#define HORIZONTAL_TOOLBAR_STYLESHEET "QToolBar {\
    border: 0px solid #FAFAFA;\
    background: 1px solid;\
    font-weight: bold;\
    QToolButton {border-left-width: 6px; color: rgb(92,81,92);}\
    QToolButton:checked {\
    border-left-color: #FE94CD; border-left-style: solid; border-left-width: 6px;\
    color: #121012;\
    }\
}"
...
{
    resize(1000, 650);
    setWindowTitle(tr("IDOL") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/IDOL"));
    setWindowIcon(QIcon(":icons/IDOL"));
#else
...
void BitcoinGUI::createActions()
{
    ...
    blockAction->setToolTip(tr("Explore the IDOLCOIN Blockchain"));
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a IDOLCOIN address"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for IDOLCOIN"));
...
void BitcoinGUI::ActionSetName(QToolBar * bar, QAction * action, const char * name) {
...
void BitcoinGUI::createToolBars()
{
    ...
    ActionSetName(mainToolbar, overviewAction, "overviewAction");
    ActionSetName(mainToolbar, sendCoinsAction, "sendCoinsAction");
    ActionSetName(mainToolbar, receiveCoinsAction, "receiveCoinsAction");
    ActionSetName(mainToolbar, historyAction, "historyAction");
    ActionSetName(mainToolbar, mintingAction, "mintingAction");
    ActionSetName(mainToolbar, addressBookAction, "addressBookAction");
    ActionSetName(mainToolbar, statisticsAction, "statisticsAction");
    ActionSetName(mainToolbar, blockAction, "blockAction");
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("IDOLCOIN client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("IDOLCOIN client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to IDOLCOIN network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid IDOLCOIN address or malformed URI parameters."));
    }
...
QString BitcoinGUI::ThemeColor(const char* name,const char* theme) {
    char str[1024] = "";
    strcat(str, "QToolButton#");strcat(str, name);strcat(str, ":checked {border-left-color:");strcat(str, theme);
    strcat(str, "; border-left-style: solid; border-left-width: 6px;color: ");strcat(str, theme);strcat(str, ";}");
    strcat(str, "QToolButton#");strcat(str, name);strcat(str, ":pressed {border-left-width: 6px;color: ");strcat(str, theme);strcat(str, ";}");
    strcat(str, "QToolButton#");strcat(str, name);strcat(str, ":hover {border-left-width: 6px;color: ");strcat(str, theme);strcat(str, ";}");
    return (QString)str;
}
void BitcoinGUI::mainToolbarOrientation(Qt::Orientation orientation)
{
    ...
    {
        QString theme = VERTICAL_TOOBAR_STYLESHEET
        + ThemeColor("overviewAction","#ff8c36")
        + ThemeColor("sendCoinsAction","#f5c236")
        + ThemeColor("receiveCoinsAction","#ff4489")
        + ThemeColor("historyAction","#ff62be")
        + ThemeColor("mintingAction","#09beff")
        + ThemeColor("addressBookAction","#5a84da")
        + ThemeColor("statisticsAction","#1ae541")
        + ThemeColor("blockAction","#cf6ffc");

        mainIcon->setPixmap(QPixmap(":images/vertical"));
        mainIcon->show();
        mainToolbar->setStyleSheet(theme);
    }
}
